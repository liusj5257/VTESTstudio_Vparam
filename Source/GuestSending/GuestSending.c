...
#define GearLvrIndcn data[i][0]
#define DrvrSeatSts data[i][1]
#define PassSeatSts2 data[i][2]
#define Res data[i][3]

char gFname[] = __FILE__;

int main() {
  gFunction = GuestSending;
...
void adjust_data(int **data) {
  int i = 0;
  for (i = 0; i < gRow; i++) {
    if (PassSeatSts2 == 1 && DrvrSeatSts == 1 && GearLvrIndcn == 0)
      Res = 1;
    else
      Res = 0;
    ;
